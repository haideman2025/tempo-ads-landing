...
const visualDiary = [
  ...
  { src: ASSETS.ritualPreparation, index: "03", kicker: "THE PAUSE", title: "Chọn một điểm dừng" },
  { src: ASSETS.coupleKitchen, index: "04", kicker: "THE MOMENT", title: "Có mặt cho nhau" },
  { src: ASSETS.lifestyleTogether, index: "05", kicker: "THE NIGHT", title: "Giữ nhịp vừa đủ" },
...
function EditorialVideo() {
  ...
  return <div className="video-frame" data-playback-state={playbackState}>
    ...
    <div className="video-frame__label"><span>MOTION / 01</span><span>THE FIRST PAUSE</span></div>
  </div>;
}
...
function MotionCarousel() {
  ...
  return <section className="motion-carousel" aria-label="Bốn chương video tiếp theo của câu chuyện TEMPO" aria-roledescription="carousel">
    <div className="motion-carousel__masthead"><p className="overline">MOTION STORY / 02 → 05</p><h2>Một câu chuyện.<br /><em>Đi tiếp theo nhịp.</em></h2><p>Video tự phát tuần tự khi bạn đang xem; bạn vẫn có thể vuốt, chạm timeline hoặc dừng lại ở một khung hình.</p></div>
    ...
    <div className="motion-carousel__stage" data-playback-state={reducedMotion ? "reduced-motion" : playbackState} data-swipe="enabled" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="motion-carousel__copy"><span>CHƯƠNG {active.step} / 05</span><h3>{active.title}</h3><p>{active.caption}</p><small>{playbackState === "playing" ? "Đang phát tự động" : playbackState === "paused" ? "Đã dừng" : "Đang tải cảnh"}</small></div>
...
export default function Home() {
  ...
  return <div className="night-site">
    <header className="night-header">
      ...
      <div className="header-center"><span>Story 01 → 05</span><i /> <span>Danh sách chờ 01.000</span></div>
    ...
    <main id="top">
      <section className="hero-night">
        ...
        <div className="hero-copy">
          <p className="overline">V2JOY / NIGHT CONFIDENT / STORY 01</p>
        ...
        <div className="hero-footer"><span>VIDEO 01 / TỰ PHÁT</span><Signal /><span>TEMPO 3ML · 5ML</span></div>
      </section>

      <section className="commerce-intro" id="story" aria-labelledby="why-title">
        <div><p className="chapter">WHY / 01 · THE PAUSE</p><p className="commerce-intro__eyebrow">TRƯỚC KHI NÓI VỀ SẢN PHẨM</p></div>
      ...
      <section className="cinema-section cinema-section--lifestyle">
        ...
        <div className="cinema-section__copy"><p className="overline">HOW / SET THE TONE</p><h2>Một khoảng dừng.<br /><em>Một ý định rõ.</em></h2><p>Không đưa sự vội vàng vào một buổi tối dành cho hai người. Hãy bắt đầu bằng những điều nhỏ mà bạn có thể chủ động chuẩn bị.</p><span className="micro-note">TỰ CHỌN NHỊP / TỰ ĐỌC THÔNG TIN / TỰ QUYẾT ĐỊNH</span></div>
      </section>

      <section className="story-panel story-panel--pocket">
        <div className="story-image"><SafeImage src={ASSETS.pack3mlVerified} alt="TEMPO 3ml với nhãn TEMPO rõ ràng trên thân chai và hộp" /><span className="image-index">3ML / CARRY THE SIGNAL</span></div>
        <div className="story-copy"><p className="overline overline--dark">HOW / FORMAT 01</p><h2>Nhỏ để mang theo.<br /><em>Rõ để tự chọn.</em></h2><p>TEMPO 3ml là format gọn cho người muốn bắt đầu tìm hiểu. Bạn có thể chọn mối quan tâm trong form và đợi thông tin mở bán chính thức.</p><button onClick={() => { setPreferredSku("3ml"); goToWaitlist(); }} type="button" className="text-button">Quan tâm TEMPO 3ml <ArrowUpRight size={16} /></button></div>
      </section>

      <MotionCarousel />

      <section className="ingredient-atlas" aria-labelledby="ingredient-atlas-title">
        <div className="ingredient-atlas__heading"><div><p className="overline overline--dark">HOW / INGREDIENTS MADE LEGIBLE</p><h2 id="ingredient-atlas-title">Thảo dược có thể xem.<br /><em>Thông tin có thể đọc.</em></h2></div><p>Chúng tôi trình bày thành phần bằng hình ảnh để dễ tiếp cận hơn, nhưng luôn giữ phần quan trọng nhất ở chữ: danh mục INCI, nhãn thành phẩm và hồ sơ chính thức.</p></div>
        <div className="ingredient-atlas__grid">{ingredientInfographics.map(item => <article key={item.step}><SafeImage src={item.image} alt={item.alt} /><div><span>{item.step} / INGREDIENT ATLAS</span><h3>{item.title}</h3><p>{item.copy}</p></div></article>)}</div>
      ...
      <section className="transparency-protocol" id="nguon-goc" aria-labelledby="transparency-title">
        <div className="transparency-protocol__masthead"><div><p className="overline overline--dark">HOW / READ BEFORE YOU TRUST</p><h2 id="transparency-title">Không cần tin vội.<br /><em>Hãy đọc cùng chúng tôi.</em></h2></div><p>TEMPO là mỹ phẩm. Mọi thông tin dưới đây được trình bày để bạn tự đối chiếu trước khi đăng ký nhận tin mở bán.</p></div>
        <div className="transparency-hero"><SafeImage src={ASSETS.botanicalLedger} alt="Sổ ghi chép thành phần và vật liệu thực vật minh hoạ nguyên tắc đối chiếu công thức" /><div className="transparency-hero__copy"><span>01 / FORMULA</span><h3>Danh mục rõ ràng.<br /><em>Không cần đoán.</em></h3><p>INCI mở; thành phần có Alcohol (Ethanol), Butylene Glycol, chín chiết xuất thực vật, Sodium Benzoate và Panthenol.</p><a href="#inci-full" className="transparency-link">Xem INCI đầy đủ <ArrowDownRight size={16} /></a></div></div>
        <div className="transparency-steps">
          <article className="transparency-card transparency-card--origin"><SafeImage src={ASSETS.botanicalExtraction} alt="Không gian chiết xuất đương đại minh hoạ quy trình đối chiếu thông tin sản phẩm" /><div><span>02 / MANUFACTURER</span><h3>Sản xuất tại<br /><em>Việt Nam.</em></h3><p><b>Công ty TNHH SX Công nghệ cao NANOFRANCE</b><br />KCN Đồng Văn IV, Ninh Bình.</p></div></article>
          <article className="transparency-card transparency-card--label"><SafeImage src={ASSETS.botanicalTraditional} alt="Vật liệu thực vật và giấy ghi chép minh hoạ bước đọc nhãn sản phẩm" /><div><span>03 / LABEL</span><h3>Đọc nhãn trước<br /><em>khi bắt đầu.</em></h3><p>Tên SKU · dung tích · INCI · số lô · NSX/HSD · hướng dẫn.</p></div></article>
          <article className="transparency-card transparency-card--proof"><SafeImage src={ASSETS.botanicalNight} alt="Tĩnh vật thảo mộc trong ánh sáng teal dịu minh hoạ nguyên tắc minh bạch hồ sơ" /><div><span>04 / OPEN FILE</span><h3>Hồ sơ mở bán<br /><em>được cập nhật.</em></h3><p>Nhãn thành phẩm và hồ sơ chính thức tại thời điểm mở bán là nguồn thông tin áp dụng ưu tiên.</p></div></article>
        </div>
        <div className="transparency-inci" id="inci-full"><div><span>INCI / REFERENCE LIST</span><h3>Toàn bộ tên thành phần<br /><em>theo danh mục công bố.</em></h3></div><details><summary>Hiện danh sách INCI <ChevronDown size={16} /></summary><p>Purified Water, Alcohol (Ethanol), Butylene Glycol, Cnidium monnieri extract, Zanthoxylum bungeanum extract, Sophora flavescens extract, Polygonatum sibiricum extract, Eucommia ulmoides extract, Cynomorium songaricum extract, Epimedium brevicornum extract, Verbena officinalis extract, Morinda officinalis extract, Sodium Benzoate, Panthenol.</p></details></div>
      ...
      <section className="product-protocol" aria-labelledby="protocol-title">
        <div className="product-protocol__heading"><p className="overline">WHAT / PRODUCT CARE ROUTINE</p><h2 id="protocol-title">Một cách chuẩn bị rõ ràng.<br /><em>Không cần nói quá.</em></h2><p>Hướng dẫn dưới đây là nội dung công bố trên nhãn. Luôn đọc nhãn thành phẩm và thực hiện theo thông tin được cập nhật chính thức.</p></div>
      ...
      <section className="product-block" id="san-pham">
        <div className="product-block__top"><div><p className="overline overline--dark">WHAT / CHOOSE YOUR FORMAT</p><h2>Chọn format<br /><em>đi cùng bối cảnh.</em></h2></div><p>Bốn lựa chọn dưới đây giúp bạn cho V2JOY biết format mình quan tâm. Đây chưa phải đơn hàng hay thông báo giá cuối cùng.</p></div>
      ...
      <section className="visual-proof" aria-label="Mạch hình ảnh Night Confident">
        <div className="visual-proof__heading"><p className="overline">THE VISUAL DIARY / 06 CHAPTERS</p><h2>Không chỉ một ảnh.<br /><em>Mà là cả một buổi tối.</em></h2><p>Sáu khung hình nối nhau để kể bối cảnh; sản phẩm chỉ xuất hiện khi nó thật sự liên quan đến lựa chọn của bạn.</p></div>
      ...
      <section className="faq-section"><div><p className="overline overline--dark">BEFORE YOU CHOOSE</p><h2>Biết trước,<br /><em>chọn kỹ hơn.</em></h2></div><div className="faq-list">{[
